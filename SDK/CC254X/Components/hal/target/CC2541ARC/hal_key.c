...
#define HAL_KEY_MIN_NUM_ROWS        4
#define HAL_KEY_MAX_NUM_ROWS        7
#define HAL_KEY_MIN_NUM_COLUMNS     0
#define HAL_KEY_MAX_NUM_COLUMNS     3

/* The following define which port pins are being used by keypad service */
#define HAL_KEY_P0_GPIO_PINS  ( HAL_KEY_BIT4 | HAL_KEY_BIT5 | HAL_KEY_BIT6 | HAL_KEY_BIT7 )
#define HAL_KEY_P1_GPIO_PINS  ( HAL_KEY_BIT0 | HAL_KEY_BIT1 | HAL_KEY_BIT2 | HAL_KEY_BIT3 )

/* These defines indicate the direction of each pin */
#define HAL_KEY_P0_INPUT_PINS   ( HAL_KEY_BIT4 | HAL_KEY_BIT5 | HAL_KEY_BIT6 | HAL_KEY_BIT7)
//#define HAL_KEY_P0_OUTPUT_PINS  ( HAL_KEY_BIT3 | HAL_KEY_BIT4 )
//#define HAL_KEY_P1_INPUT_PINS   0x00
#define HAL_KEY_P1_OUTPUT_PINS  ( HAL_KEY_BIT0 | HAL_KEY_BIT1 | HAL_KEY_BIT2 | HAL_KEY_BIT3)

/* Which pins are used for key interrupts */
#define HAL_KEY_P0_INTERRUPT_PINS   ( HAL_KEY_BIT4 | HAL_KEY_BIT5 | HAL_KEY_BIT6 | HAL_KEY_BIT7)
#define HAL_KEY_P1_INTERRUPT_PINS   ( HAL_KEY_BIT0 | HAL_KEY_BIT1 | HAL_KEY_BIT2 | HAL_KEY_BIT3)									
...
uint8 HalKeyRead ( void )
{
  ...
  uint8 col=0;
  uint8 keys=0, outputkeys=0, colcode = HAL_KEY_MAX_NUM_COLUMNS, rowcode = HAL_KEY_MAX_NUM_ROWS;

#if (HAL_KEY == TRUE)

  // Disable interrupt, as interrupt can be triggered without key press during
  // scanning process
  //P1 &= (uint8) ~HAL_KEY_OUTPUT_GPIO_PINS;
  P0IEN &= ~(HAL_KEY_P0_INTERRUPT_PINS);
  //P1IEN &= ~(HAL_KEY_P1_INTERRUPT_PINS);
  //IEN2 &= ~(HAL_P1_KEY_BIT4);

  // Detect row first while all columns are still asserted.
  keys = (P0 & HAL_KEY_P0_INPUT_PINS);
  for (uint8 row = HAL_KEY_MIN_NUM_ROWS; row <= HAL_KEY_MAX_NUM_ROWS; row++)
  {
    if (((1 << row) & keys) == 0)
    {
      if (++rowkeys == 1)
  ...
  for (col = HAL_KEY_MIN_NUM_COLUMNS; col <= HAL_KEY_MAX_NUM_COLUMNS; col++)
  {
    P1 |= ((uint8)1 << col);
    // read all rows
    outputkeys = (P0 & HAL_KEY_P0_INPUT_PINS);

    // de-assert the column
    //halClockShiftRegister();

    // check whether the specific row was on.
    if (outputkeys & ((uint8)1 << rowcode))
    {
      if (++colkeys == 1)
      {
  ...
  if ((colcode > HAL_KEY_MAX_NUM_COLUMNS) ||
      (rowcode > HAL_KEY_MAX_NUM_ROWS)    
     //   ||(rowkeys > 1)                    ||
 //     (colkeys > 1)
        )
  {
