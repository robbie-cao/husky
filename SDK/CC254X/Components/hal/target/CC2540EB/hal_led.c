...
void HalLedEnterSleep( void )
{
    /*
     * ATTENTION:
     * The following commented lines will cause LED on
     * P1_0 and P1_1 flash when pressing keyboard with
  ...
  HalSleepLedState |= HAL_STATE_LED3() << 2;        // FIXME: Comment out this line will cause UART work abnormal, WHY???
  //HalSleepLedState |= HAL_STATE_LED4() << 3;

  /* TURN OFF all LEDs to save power */
  HalLedOnOff (HalSleepLedState, HAL_LED_MODE_OFF);
#endif /* HAL_LED */
